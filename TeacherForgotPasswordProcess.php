    ...
    if(empty($email)){
        ...
        if($rs->num_rows == 1){
            ...
            Database::iud("UPDATE `teacher` SET `code` = '".$cord."' WHERE `Email` = '".$email."' ");
             ...
             $mail->SMTPSecure = 'ssl';
             $mail->Port = 465;
             $mail->setFrom('aatzan30@gmail.com', 'Teacher Portal');
             $mail->addReplyTo('aatzan30@gmail.com', 'Teacher Portal');
             $mail->addAddress($email);
             $mail->isHTML(true);
             $mail->Subject = 'Teacher Portal Forgot Password Verification Code.';
