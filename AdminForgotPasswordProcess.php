...
if(isset($_GET["email"])){
    ...
    if(empty($email)){
        ...
        if($rs->num_rows == 1){
            ...
            Database::iud("UPDATE `admin` SET `code` = '".$cord."' WHERE `Email` = '".$email."' ");
             ...
             $mail->SMTPSecure = 'ssl';
             $mail->Port = 465;
             $mail->setFrom('aatzan30@gmail.com', 'Admin Portal');
             $mail->addReplyTo('aatzan30@gmail.com', 'Admin Portal');
             $mail->addAddress($email);
             $mail->isHTML(true);
             $mail->Subject = 'Admin Portal Forgot Password Verification Code.';
