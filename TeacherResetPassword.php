<?php
    ...
    if(empty($email)){
        ...
        $rs = Database::search("SELECT * FROM `teacher` WHERE `Email` = '".$email."' AND `code` = '".$verificationCode."' ");

        if($rs->num_rows == 1){

            Database::iud("UPDATE `teacher` SET `password` = '".$newPassword."' WHERE `Email` = '".$email."' ");
            echo "success";
