...
if(isset($_GET["email"])){
    ...
    if(empty($email)){
        ...
        if($rs->num_rows == 1){
            ...
            Database::iud("UPDATE `student` SET `code` = '".$cord."' WHERE `Email` = '".$email."' ");
             ...
             $mail->SMTPSecure = 'ssl';
             $mail->Port = 465;
             $mail->setFrom('aatzan30@gmail.com', 'Student Portal');
             $mail->addReplyTo('aatzan30@gmail.com', 'Student Portal');
             $mail->addAddress($email);
             $mail->isHTML(true);
             $mail->Subject = 'Student Portal Forgot Password Verification Code.';
