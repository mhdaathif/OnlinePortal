<?php
    ...
    if(empty($email)){
        ...
        $rs = Database::search("SELECT * FROM `academicofficer` WHERE `Email` = '".$email."' AND `code` = '".$verificationCode."' ");

        if($rs->num_rows == 1){

            Database::iud("UPDATE `academicofficer` SET `password` = '".$newPassword."' WHERE `Email` = '".$email."' ");
            echo "success";
